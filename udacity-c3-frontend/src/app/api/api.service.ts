...
export class ApiService {
  ...
  get(endpoint, feed?: boolean): Promise<any> {
    const url = feed ? `${API_FEED_HOST}${endpoint}` : `${API_HOST}${endpoint}`;
    const req = this.http
  ...
  post(endpoint, data, feed?: boolean): Promise<any> {
    const url = feed ? `${API_FEED_HOST}${endpoint}` : `${API_HOST}${endpoint}`;
    return this.http
  ...
  async upload(endpoint: string, file: File, payload: any): Promise<any> {
    const signed_url = (await this.get(
      `${endpoint}/signed-url/${file.name}`,
      true
    )).url;
    ...
    return new Promise(resolve => {
      this.http.request(req).subscribe(resp => {
        if (resp && (<any>resp).status && (<any>resp).status === 200) {
          resolve(this.post(endpoint, payload, true));
        }
      });
    });
  }
